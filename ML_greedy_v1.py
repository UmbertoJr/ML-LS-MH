...
class MLGreedy:
    ...
    @staticmethod
    def two_opt_reduced_CL(X, free_nodes, fixed_edges, distance_matrix, CLs, tour, tabu_list=False):
        # tour = create_tour_from_X(X)
        # assert check_feasibility(tour), "Problem the tour is not feasible"
        n = len(tour)
        ops = 0
        for i, node_ip in enumerate(tour[:-1]):
            ...
            if node_ip in free_nodes and node_in in free_nodes and (node_ip, node_in) not in fixed_edges:
                for node_jp in CLs[node_ip]:
                    ...
                    if i < j:
                        node_jn = tour[j + 1- n]
                        id_first_node = min(node_ip, node_in)
                        id_second_node = max(node_ip, node_in)
                        if tabu_list.get((id_first_node, id_second_node), 0) != 0:
                            tabu_list[(id_first_node, id_second_node)] -= 1
                            continue
                        ...
                        if node_jp in free_nodes and node_jn in free_nodes and (node_jp, node_jn) not in fixed_edges:
                            ...
                            if old_cost - new_cost > 0:
                                ...
                                if tabu_list.get((id_first_node, id_second_node), 0) == 0:
                                    tabu_list[(id_first_node, id_second_node)] = 100
                                    return X, old_cost - new_cost, ops, create_tour_from_X(X), tabu_list
                                    
                                return X, old_cost - new_cost, ops, create_tour_from_X(X), tabu_list
        
        # check if tabu list is a dictionary
        if type(tabu_list) == dict:
            for key in tabu_list:
                tabu_list[key] -= 1
            return X, 0, ops, tour, tabu_list
    ...
    def run_ILS(X, distance_matrix, free_nodes, fixed_edges, CLs, type_2opt="reduced", opt_len=None):
        ...
        total_iterations_available = 100000000
        ...
        while True:
            ...
            X_c, improvement, ops_plus, current_tour, tabu_list = two_opt_fun(X_c, free_nodes,
                                                                              fixed_edges, distance_matrix, 
                                                                              CLs, current_tour, tabu_list=tabu_list)
            ops_used += ops_plus 
        ...
        while avg_probs>0.01:
            ...
            while True:
                ...
                X_proposal, improvement, ops_plus, tour_proposal, tabu_list = two_opt_fun(X_proposal, free_nodes, 
                                                                                            fixed_edges,
                                                                                            distance_matrix, CLs, 
                                                                                            tour_proposal, tabu_list=tabu_list)
                ops_used += ops_plus
            ...
            if counter_temperature>repeat_temperature:
                ...
                # if the len of the seen tours is too big it removes form the hashtable the tours that have been seen less than 3 times
                # if len(seen_tours)>1000000:
                #     for k, v in seen_tours.items():
                #         if v<3:
                #             del seen_tours[k]
                    # print(f"Process {mp.current_process().name},  dimension of the hashtable {len(seen_tours)}")
...
def standardize_tour(tour, free_nodes, segment_length=4, both_tours=True):
    """
    It takes a tour as a list then just for the free nodes it finds the segments of length "segment_length" that start from the freenode in both directions.
    Then it saves the segments, for eaxh free node, in a dictionary and then it returns the dictionary. The segments are saved as a string.
    """
    new_connections = {}
    for node in free_nodes:
        idx = np.argwhere(tour[:-1] == node)[0][0]
        tour_to_return = np.roll(tour[:-1], -idx)
        tour_to_return = np.append(tour_to_return, tour_to_return[0])
        string_tour = ""
        for node in tour_to_return:
            string_tour += str(node) + "-"
        string_tour = string_tour[:-1]
        new_connections[node] = string_tour
        if both_tours:
            string_tour_2 = ""
            for node in tour_to_return[::-1]:
                string_tour_2 += str(node) + "-"
            string_tour_2 = string_tour_2[:-1]
            new_connections[node] = string_tour_2
